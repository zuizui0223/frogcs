...
1. **Short environmental pulses can change community observations without changing the underlying species pool, yet the spatial level at which that change enters the community is rarely resolved.** Rainfall effects on frog calling are well established. We therefore asked a different question: does recent rain primarily activate additional sites, deepen local assemblages, expand landscape-scale richness, or reorganize compositional differences among sites?

2. **We analysed 15 years of standardized North American Amphibian Monitoring Program surveys as a spatially replicated active-community system.** We compared 4,236 wetter–drier survey pairs from the same route and seasonal sampling window across adjacent observed years. Each complete route comprised ten aligned stops, allowing us to partition change among the active spatial footprint, local richness (alpha), route-level richness (gamma), among-active-stop beta diversity, and the underlying species × stop incidence matrix.

3. **Recent rain was associated with simultaneous spatial and taxonomic expansion.** Greater rainfall contrast predicted more active stops (β = 0.384 stops, 95% CI 0.240–0.528), higher richness per active stop (β = 0.079 species, 95% CI 0.023–0.136), and higher route-level active richness (β = 0.286 species, 95% CI 0.165–0.407). Richness also increased within the same numbered stops that were active in both paired surveys (β = 0.095 species per stop, P = 0.0020).

4. **Expansion did not measurably erase spatial community structure.** Rainfall contrast had near-zero effects on mean pairwise Sørensen dissimilarity among active stops (β = -0.00049, P = 0.937), normalized Whittaker beta diversity (β = -0.00482, P = 0.339), turnover or nestedness. An exact decomposition of the rainfall-associated increase in species × stop incidences showed that 36.9% came from route-new species at newly active stops, 15.2% from route-existing species spreading to newly active stops, 39.9% from route-new species entering already-active stops, and only 8.0% from rearrangement of route-existing species among already-active stops.

5. **Taxonomic expansion expanded taxonomically without detectable change in measured functional dispersion or centroid.** Trait-covered richness increased, whereas mean pairwise functional distance and community-weighted means across body size, clutch size, offspring size and reproductive output did not detectably shift. Among 24 species, functional distance did not predict rainfall-response dissimilarity (permutation P = 0.797; within-family P = 0.739), and a temporally held-out test did not support richness buffering by rainfall response diversity. We conclude that recent rainfall is associated with a two-dimensional expansion of the behaviourally realized frog community—across sites and species—without detectable spatial homogenization or a simple shift along conventional life-history trait axes. These are acoustic-activity patterns, not evidence of occupancy change or rainfall causality.

## Keywords

active community; alpha diversity; beta diversity; environmental pulse; functional diversity; frog chorus; gamma diversity; metacommunity; rainfall; response diversity
...
### Taxonomic expansion expands taxonomically without detectable change in measured functional dispersion or centroid
...
**Figure 3. Taxonomic, functional and response dimensions are partly decoupled.** Panel A contrasts the positive trait-covered richness coefficient with near-zero functional MPD and CWM shifts. Panel B plots species-pair functional distance against rainfall-response dissimilarity with the permutation result. Panel C summarizes the distinction between functional diversity and rainfall response diversity and notes that the held-out richness-buffering prediction was unsupported.
