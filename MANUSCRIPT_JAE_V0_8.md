...
1. **Short environmental pulses can reorganize observed animal communities without demographic turnover, but the spatial scale at which those changes enter a community is often unresolved.** Rainfall effects on frog calling are well known; we asked whether recent rain expands the active spatial footprint, deepens local assemblages, increases landscape richness or alters spatial differentiation.
...
3. **Rainfall contrast was associated with expansion at both spatial and taxonomic scales.** Greater contrast predicted more active stops (β = 0.384, 95% CI 0.240–0.528), higher richness per active stop (β = 0.079, 0.023–0.136) and higher route richness (β = 0.286, 0.165–0.407). These signals persisted after adjustment for recorded hearing impairment, major-noise interruptions and wind.
...
5. **Species also differed repeatably in how their gains entered the matrix.** Opportunity-corrected activation geometry was strongly correlated across non-overlapping periods (ρ = 0.774, P = 0.00044), and 15 of 16 species retained the same spatial-edge versus local-deepening sign. Conventional life-history traits did not strongly encode this response trait. Recent rainfall therefore expands the behaviourally realized frog community through repeatable species-specific activation modes, without evidence of occupancy change or rainfall causality.

## Keywords

active community; alpha diversity; beta diversity; environmental pulse; functional diversity; metacommunity; rainfall; response diversity
...
Functional traits provide a second axis of interpretation. Trait-based community ecology often asks whether environmental variation selects particular ecological strategies, but trait shifts alone are insufficient to establish environmental filtering in a strict assembly sense (Kraft et al., 2015). Here the more limited question is whether rainfall-associated taxonomic expansion is accompanied by displacement of the community in a conventional life-history trait space. A further distinction is between **functional diversity** and **response diversity**. Response diversity describes variation among species in their reactions to environmental change and can, in some systems, contribute to aggregate stability (Mori et al., 2013; Hsieh et al., 2026). Importantly, response diversity can be quantified either indirectly from low-level traits assumed to encode environmental sensitivity or more directly from species' observed environment-dependent responses; the latter avoids assuming that conventional functional traits are valid response proxies (Ross et al., 2023). Recent work also shows that response diversity need not stabilize communities under pulse disturbances, because stability can depend more strongly on mean species responses and species interactions (Kunze et al., 2026). This raises two empirical questions: do species that are similar in familiar life-history traits also respond similarly to rainfall, and can a higher-level response trait be measured directly from how species enter the active community matrix?

We addressed these questions using standardized North American Amphibian Monitoring Program (NAAMP) surveys. We treated each ten-stop route-run as a landscape sampling unit containing replicated local active assemblages; we use “metacommunity-scale” and “active metacommunity” as descriptions of this spatial organization, not as evidence of demographic connectivity among stops. We matched wetter and drier runs from the same route and seasonal sampling window, then quantified (i) the number of active stops, (ii) local alpha richness among active stops, (iii) route-level gamma richness, (iv) among-active-stop beta diversity, and (v) exact changes in the species × stop incidence matrix. We then asked whether taxonomic expansion altered a frozen four-axis life-history trait space and whether functional similarity predicted similarity in species-specific rainfall responses. Finally, we tested whether species possess a repeatable **activation geometry**: an opportunity-corrected tendency for wetter-condition gains to enter newly active sites versus sites that were already active. The central prediction was not simply that recent rain would increase activity, but that its community-scale signature could be localized to particular spatial and taxonomic components and, potentially, to repeatable species-specific response modes.
...
### Species-specific activation geometry as a response trait

The four-way incidence decomposition suggested that species could differ not only in the magnitude of their wetter-versus-drier response, but also in **where** their gained incidences entered the active matrix. We therefore defined an opportunity-corrected species response trait and tested it across non-overlapping periods.

For each wet-gain species × stop incidence, we recorded whether the stop had been acoustically inactive in the paired drier run. Because species have more opportunities to gain at inactive stops when a drier run contains many such stops, each matched pair was assigned the available inactive-stop fraction, `q`. Pairs with `q = 0` or `q = 1` contain no spatial-choice information and were structurally excluded. For each species and period we fitted a binomial model to counts of wet-gain incidences at newly active versus already-active stops, using `logit(q)` as an offset and route-clustered covariance. The fitted intercept is the species' **activation geometry log odds**: positive values indicate wetter-condition gains disproportionately entering newly active sites (“spatial-edge activators”), whereas negative values indicate gains disproportionately entering sites that were already active (“local taxonomic deepeners”).

We split the record into 2001–2007 and 2008–2015. Within each period, a species required at least 30 informative wet-gain incidences across at least 15 matched pairs and ten routes. The primary temporal-validation family was the intersection of species estimable in both periods, with a frozen minimum of 15 species. Primary repeatability was Spearman rank correlation between early- and late-period activation geometry. Prespecified sensitivities were inverse-variance weighted regression of late on early geometry and sign concordance relative to 0.5. This is a response trait of acoustic gain placement, not occupancy niche breadth, dispersal strategy or colonization.

As a separately frozen cross-axis diagnostic, we then asked whether early-period activation geometry predicted the **magnitude** of the species' adjusted wet-versus-dry response in 2008–2015. Response magnitude was rebuilt from late-period discordant matched pairs using the same adjusted species model used in the held-out spatial-breadth analysis. We reported a two-sided Spearman correlation without a directional prediction and prespecified |ρ| ≥ 0.6 with P < 0.05 as evidence of strong coupling. An inverse-variance weighted regression of later response magnitude on earlier geometry was secondary. This diagnostic does not test statistical independence or equivalence.
...
We derived four axes: log body size; mean log clutch size from litter-size bounds; mean log offspring size from offspring-size bounds; and log reproductive output per year. Each axis was standardized across complete species. Functional distance was Euclidean distance in the four-dimensional standardized space divided by (sqrt{4}).

For each route-run we calculated trait-covered richness, mean pairwise functional distance (MPD), and community-weighted mean values of each raw derived trait. For matched pairs we also calculated a **functional novelty balance**: the mean nearest-neighbour trait distance of wet-only species to the drier assemblage minus the symmetric distance of dry-only species to the wetter assemblage. MPD was repeated with trait-covered richness difference included as a sensitivity covariate. Benjamini–Hochberg correction was applied across the four community-weighted-mean tests.

This trait space represents a defined set of life-history and reproductive axes; it is not exhaustive ecological function.

### Functional similarity versus rainfall-response similarity

Species-specific rainfall responses were estimated within the fixed, direction-blind species family requiring at least 40 discordant matched-pair events across at least ten routes. For each eligible species, discordant events were coded 1 when the species occurred only in the wetter run and 0 when only in the drier run. We fitted

`wet_only ~ centered rain_contrast + centered temperature_difference + centered day_of_year_difference + centered year_gap`

with route-clustered covariance. The intercept estimates adjusted wet-versus-dry response at the species’ mean paired conditions.

Among species with both an adjusted response and complete functional traits, we calculated all pairwise functional distances and absolute pairwise rainfall-response differences. Their Pearson correlation was evaluated with 100,000 unrestricted permutations of response labels among species. A second 100,000-permutation sensitivity shuffled response labels within AmphiBIO family, asking whether any alignment remained beyond broad taxonomic structure. Because species pairs are non-independent, inference came from label permutation rather than naive pairwise regression.

### Temporally held-out response-diversity buffering test

To test a stronger response-diversity hypothesis without reusing the same outcomes for predictor construction and validation, we split the record into 2001–2007 and 2008–2015. Species wet-versus-dry response signs were estimated only in the early period, using a minimum of 25 discordant events across eight routes.

For each route × RunNumber early-period species pool, response-sign diversity was

`4 p (1 - p)`,

where (p) is the fraction of scored species with positive early-period rainfall responses. This ranges from 0 when all species share the same sign to 1 at equal positive and negative proportions. Pools required at least three scored species and coverage of at least 60% of the early species pool.

We then asked whether early response-sign diversity reduced the sensitivity of late-period route richness to rainfall contrast using the interaction

`rain_contrast × response_sign_diversity`

while also allowing rainfall interactions with mean early response and scored richness. The prespecified buffering prediction was a negative interaction with a 95% confidence interval excluding zero. Exact consecutive-year late pairs were a sensitivity analysis.
...
Previously completed post-opening extensions evaluated rain-recency bins, between-year turnover and species-specific wet-versus-dry heterogeneity. We retain these as secondary context rather than the manuscript’s primary inferential spine. The run-level timescale analysis compared day 0, day 1, days 2–3 and days 4–7 with a sparse ≥8-day reference. Although all four route-season fixed-effect contrasts were positive, the stricter exact-consecutive-year matched contrast was precise through days 2–3 and imprecise at days 4–7. We therefore do not use “week-long pulse” as a headline inference.
...
### Species have repeatable activation geometry

Activation geometry was estimable for 17 species in 2001–2007 and 31 species in 2008–2015; 16 species met the frozen eligibility criteria in both non-overlapping periods. Species rankings were strongly repeatable (Spearman ρ = 0.774, P = 0.000439). An inverse-variance weighted regression gave a late-on-early slope of 0.910 (95% CI 0.816–1.003), and 15 of 16 species retained the same sign (93.8%; one-sided exact binomial P = 0.000259).

The trait separated taxa that consistently placed wetter-condition gains at different parts of the active matrix. *Pseudacris crucifer* was a strong spatial-edge activator in both periods (early geometry = +1.144; late = +1.061), whereas *Lithobates palustris* was a strong local taxonomic deepener (early = -0.967; late = -0.702). *Lithobates catesbeianus* was also consistently on the local-deepening side (early = -0.139; late = -0.335). Only one of the 16 overlap species changed sign.

This repeatable response geometry was not strongly aligned with the four conventional life-history traits used elsewhere in the manuscript. Descriptive correlations of early-period activation geometry with log body size, clutch size, offspring size and reproductive output were all weak to moderate and imprecise (|ρ| ≤ 0.31; all P ≥ 0.287). Family-adjusted regression was not estimable under the frozen coverage requirement, so we do not infer that activation geometry is independent of phylogenetic or family structure.

Activation geometry was also not simply identical to overall wet-versus-dry response magnitude. Across the same 16 species, early geometry had a moderate but imprecise rank association with independently estimated late-period response magnitude (Spearman ρ = 0.415, P = 0.110), below the frozen strong-coupling criterion. A precision-weighted regression was nevertheless positive (β = 0.354, 95% CI 0.204–0.503, P = 3.36 × 10^-6): species on the spatial-edge side tended, among the more precisely estimated taxa, to be more wet-associated later. We therefore treat response magnitude and activation geometry as conceptually distinct but partially coupled coordinates, not as statistically independent axes.

### Taxonomic expansion occurs without detectable shift in measured functional dispersion or centroid

The frozen functional panel captured 93.2% of eligible run × species incidences. Trait-covered richness increased strongly with rainfall contrast (β = 0.2813, 95% CI 0.1516–0.4111, P = 2.14 × 10^-5), closely tracking the full taxonomic result.

In contrast, mean pairwise distance in the four-axis functional space showed no detectable expansion (β = -0.0153, 95% CI -0.0434–0.0128, P = 0.285). Adding the change in trait-covered richness as a covariate did not alter this conclusion (P = 0.437). Community-weighted means of log body size, clutch size, offspring size and reproductive output likewise showed no supported rainfall-associated shift after multiplicity correction.

Functional novelty balance was weakly positive in the full matched analysis (β = 0.0419, 95% CI 0.00148–0.0824, P = 0.0422), but the exact consecutive-year sensitivity was imprecise (β = 0.0483, P = 0.0640). We therefore treat this as evidence that added species were not universally exact functional duplicates, while avoiding a claim that rainfall expands functional dispersion or shifts the functional centroid.

### Conventional functional similarity does not predict rainfall-response similarity

Species-specific wet-versus-dry activity responses were heterogeneous, but that heterogeneity was not organized by the frozen functional trait space. Twenty-four response-eligible species had complete functional vectors, producing 276 species pairs. The correlation between functional distance and absolute adjusted rainfall-response difference was -0.0328. Under 100,000 unrestricted permutations of response labels, the two-sided P value was 0.797.

The result was unchanged when response labels were permuted only within AmphiBIO family (P = 0.739). Opposite rainfall-response signs occurred in 36.2% of species pairs in the lowest quartile of functional distance and 40.6% in the highest quartile. Thus similarity in the measured life-history/reproductive traits was not a useful proxy for similarity in rainfall response.

### A held-out response-diversity buffering prediction is unsupported

The early-period analysis produced estimable rainfall responses for 16 species and 846 eligible route × seasonal-window pools. The independent late-period test included 779 matched pairs from 177 routes.

The prespecified interaction between rainfall contrast and early-period response-sign diversity was negative in the full validation set, as predicted, but highly imprecise (β = -0.954, 95% CI -3.267–1.359, P = 0.419). The exact consecutive-year sensitivity was also null (β = +0.219, P = 0.868). Neither a continuous response-dispersion diagnostic nor a discordant-species compensation metric supported buffering.

Rainfall response diversity is therefore present at the species level but is not shown here to stabilize route-level active richness. This distinction is consistent with the broader observation that response diversity need not confer stability under all pulse-disturbance regimes.

### Secondary timing and between-year composition results

In the separately frozen rain-recency analysis, route-season adjusted richness was higher than the ≥8-day reference on day 0 (+0.654 species), day 1 (+0.544), days 2–3 (+0.411) and days 4–7 (+0.381). However, the ≥8-day reference contained only 338 runs, and in the stricter exact consecutive-year dry-anchor sensitivity the days 4–7 contrast was +0.257 with a confidence interval spanning zero. We therefore interpret this only as evidence that the richness association is not restricted to the calendar day of rain.

Between-year compositional turnover increased with rainfall contrast only in the exact consecutive-year sensitivity; the primary turnover coefficient was positive but imprecise. These secondary results do not alter the main within-route spatial conclusion: rainfall-associated community expansion is most directly resolved by changes in active spatial footprint, local alpha, route gamma and species × stop incidence boundaries.
...
The species-level activation-geometry result provides a more proximal response trait than the static life-history axes. Species were not merely heterogeneous in whether they appeared more often under wetter conditions; they were also consistent in **how their gains entered the spatial matrix**. Some repeatedly gained at sites that had been inactive, while others repeatedly added taxonomic depth to sites that were already active. The strong temporal repeatability, near-unity weighted slope and 15/16 sign concordance suggest that this geometry captures a stable component of species response over the monitoring period. Conceptually, activation geometry is closer to the empirically derived, higher-level response traits advocated in recent response-diversity methodology than to low-level trait proxies: it is estimated directly from an environment-dependent ecological response rather than inferred from morphology or life history (Ross et al., 2023). The cross-axis holdout also shows why this quantity should not be collapsed into response magnitude. Rank coupling with later wet-versus-dry magnitude was moderate rather than strong under the frozen rule, although precision-weighted regression indicated partial positive coupling. Thus species response is better represented as at least two related coordinates—**how strongly** participation changes and **where** gains enter the spatial matrix—than as one scalar coefficient. Importantly, activation geometry is not a dispersal or colonization trait: it describes the placement of acoustic detections conditional on wetter-run gains.

More strikingly, conventional functional similarity failed to predict similarity in species’ rainfall responses. Species close together in body size, clutch size, offspring size and reproductive output were about as capable of responding in opposite directions as functionally distant species. This result helps explain why earlier attempts to identify a single simple trait mechanism were unsuccessful. It also illustrates a broader distinction between **effect/functional traits** and **response traits**: traits that position species in a conventional functional space need not encode the environmental sensitivities that organize short-term community dynamics. Response diversity can therefore be partly cryptic with respect to commonly available life-history databases (Mori et al., 2013; Hsieh et al., 2026).

We tested one tempting implication of that response diversity and did not find it. Communities containing a more even mixture of early-period wet- and dry-associated species did not show detectably weaker richness responses in an independent later period. The confidence intervals were wide, so the analysis does not prove absence of compensation; however, it does reject a strong version of the insurance interpretation for these data. That result is not anomalous in the context of pulse-disturbance theory: recent work shows that response diversity is not universally stabilizing under pulses and that mean responses and species interactions can dominate aggregate stability (Kunze et al., 2026). The value of response diversity here is therefore descriptive and mechanistic—species differ in environmental response in ways not captured by the measured functional traits—rather than demonstrated buffering of richness.
...
**Figure 1. Recent rain expands the active community at local and landscape scales without changing beta diversity.** Panel A shows the matched route × seasonal-window design. Panel B presents rain-contrast coefficients for active-stop count, local alpha, route gamma, mean pairwise Sørensen and normalized Whittaker beta. The visual contrast is expansion of footprint, alpha and gamma against near-zero beta change.

**Figure 2. Rainfall-associated expansion occurs at the boundaries of the species × stop matrix.** A conceptual species-by-site matrix illustrates the four frozen incidence components. The quantitative panel shows coefficient shares: 36.9% corner expansion, 15.2% spatial spread, 39.9% taxonomic deepening and 8.0% within-core rearrangement. A secondary inset decomposes local alpha increase into the >=2-species threshold component and multiplicity beyond the second species.

**Figure 3. Conventional functional traits and species response geometry describe different dimensions.** Panel A contrasts the positive trait-covered richness coefficient with near-zero functional MPD and CWM shifts. Panel B plots species-pair functional distance against rainfall-response dissimilarity with the permutation result. Panel C shows early- versus late-period activation geometry for the 16 temporally validated species, distinguishing spatial-edge activators from local taxonomic deepeners.

Rain-recency curves, species-specific forest plots, between-year turnover/nestedness and rejected simple trait mechanisms are moved to Supporting Information.
