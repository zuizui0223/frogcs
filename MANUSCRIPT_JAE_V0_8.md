...
### Secondary falsification analyses

Additional frozen analyses tested stronger mechanistic alternatives without allowing them to replace the main matrix results after readback. These included a temporally held-out response-sign diversity buffering test, rain-recency bins, between-year turnover/nestedness, body size, coarse hydroperiod, breeding-season breadth, baseline recurrence, seasonal concentration, historical route dryness, early spatial niche breadth and baseline spatial heterogeneity. Full specifications, sample gates and endpoint results are reported in Supporting Information Sections S2–S5. These analyses are retained as falsification evidence rather than as a second inferential spine.
...
A temporally held-out insurance test was unsupported: greater early response-sign diversity did not detectably buffer later route-level richness responses (Supporting Information S4). Repeatable response traits therefore need not imply aggregate stability under a pulse disturbance, consistent with recent theory emphasizing the roles of mean responses and species interactions (Kunze et al., 2026).
...
**Figure 2. Rainfall-associated expansion occurs at the boundaries of the species × stop matrix.** A conceptual species-by-site matrix illustrates the four frozen incidence components and shows how species-level gain placement maps onto spatial-edge versus local-deepening pathways. The quantitative panel shows coefficient shares: 36.9% corner expansion, 15.2% spatial spread, 39.9% taxonomic deepening and 8.0% within-core rearrangement. A secondary inset decomposes local alpha increase into the >=2-species threshold component and multiplicity beyond the second species.
