...
| A non-significant beta coefficient does not prove beta diversity is unchanged. | Correct. RC6 uses “no detectable shift,” not equivalence or exact preservation. A separately frozen practical-equivalence analysis may strengthen this only if its prespecified margin is passed. | Hard boundary |
...
| Species-specific rain responses are not temporally stable. | Rank repeatability is weak; sign consistency is stronger. RC6 therefore does not use a fixed species-ranking mechanism as its central claim. | Addressed |
...
> **Recent-rain surveys expand the behaviourally realized frog community along both spatial and taxonomic axes: more stops become active, more species occur within active stops and across routes, and most additional species × stop incidences open a spatial and/or taxonomic matrix boundary rather than reshuffling an unchanged core. The measured beta-diversity metrics and broad life-history functional structure show no detectable corresponding shift.**
