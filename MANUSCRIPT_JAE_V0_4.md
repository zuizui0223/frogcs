...
1. **Environmental pulses can increase multispecies activity in at least two distinct ways:** by activating more species independently or by changing residual temporal association among species. These mechanisms are often conflated when community synchrony is inferred from the number of species detected in the same observation window.

2. **We separated these possibilities using two independent continental frog-monitoring systems.** In the North American Amphibian Monitoring Program (NAAMP), we tested rainfall recency against the proportion of standardized 5-min stops containing at least two calling species. We independently tested the same directional event-level pattern in expert-validated Australian FrogID recordings linked to ERA5 precipitation. We then decomposed the NAAMP signal into general acoustic activation, active species-pool richness and co-calling remaining after marginal calling frequencies were accounted for.

3. **Recent rainfall was associated with more multispecies calling in both systems.** Across 9,399 NAAMP runs and 93,383 sampled stops, the primary rain-recency odds ratio was 0.969 per 1 SD increase in log-transformed days since rain (95% CI 0.941–0.998). On the plausible-temperature subset, the rain association remained after temperature and nonlinear day-of-year adjustment (OR 0.946, 95% CI 0.917–0.976). In 40,754 FrogID recordings, antecedent dry-spell duration likewise reduced the odds of multiple calling species (OR 0.853, 95% CI 0.827–0.879).

4. **NAAMP decomposition showed that the rainfall signal was dominated by community activation rather than stronger residual co-calling.** More recent rainfall increased the probability that any species was calling (OR for increasing dryness = 0.904, P = 4.63 × 10^-8) and was associated with greater run-level active species richness (β = -0.0187, P = 1.92 × 10^-4). By contrast, rain did not change the probability of multiple species conditional on acoustic activity (OR = 0.988, P = 0.402), pairwise network density (P = 0.724), independence-residual co-calling (P = 0.389) or mean pairwise excess covariance (P = 0.205).

5. **Shared environmental activation can therefore generate stronger multispecies calling without detectable restructuring of residual species associations.** Short-window multispecies detections should not by themselves be interpreted as temporal-niche compression, interspecific facilitation or pairwise synchrony. The observational design identifies replicated weather associations, not causal rainfall effects.
...
FrogID independently reproduced the event-level rain direction. Greater antecedent dry-spell duration was associated with a lower probability that an acoustically active recording contained multiple calling species (β = -0.1592, OR = 0.853, 95% CI 0.827–0.879, P = 1.13 × 10^-24). Recorder-clustered inference gave the same conclusion. This result establishes independent replication of the multispecies-recording pattern, but the conditional FrogID design alone does not distinguish residual interspecific association from independent increases in species-specific activity.

### Within-space robustness

The rain direction persisted when estimates used temporal variation within fixed spatial units. Within 797 repeatedly surveyed NAAMP routes (9,256 runs), greater time since rain remained negatively associated with multispecies-stop probability (β = -0.01367 on the probability scale, 95% CI -0.01998 to -0.00737, P = 2.15 × 10^-5). Within 1,071 informative FrogID ERA5 cells (40,020 recordings), the analogous coefficient was β = -0.04265 (95% CI -0.05175 to -0.03354, P = 4.30 × 10^-20). Static geographic differences in species pools are therefore insufficient to explain the replicated direction, although time-varying confounding remains possible.
